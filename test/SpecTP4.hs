...
spec = do
  ...
  describe "Videos recomendados" $ do
    ...
    describe "Agregar videos recomendados" $ do
      ...
      let coincidenVideos videosEsperados playlist = length (videos playlist) == length videosEsperados && all (flip elem (videos playlist)) videosEsperados
      ...
      it "Al agregar videos recomendados se mantienen los originales y se incorporan sólo los recomendables" $ do
        ...
        `shouldSatisfy` coincidenVideos ([videoRecomendable, videoRecomendableRelacionado1]++videos playlistBase)
      it "Un video puede agregarse como recomendado si está relacionado con 2 videos que ya fueron agregados" $ do
        -- se testea en ambos sentidos, para admitir más de una solución
        [ agregarVideosRecomendados [videoNoRecomendable,
            videoRecomendableRelacionado1, videoRecomendableRelacionado2] playlistBase,
          agregarVideosRecomendados [videoRecomendableRelacionado2,
            videoRecomendableRelacionado1, videoNoRecomendable] playlistBase ]
        `shouldSatisfy` (any (coincidenVideos ([videoNoRecomendable, videoRecomendableRelacionado1, videoRecomendableRelacionado2]++videos playlistBase)))
